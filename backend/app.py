...
from backtest.mcx_option_resolver import get_nearest_expiry, get_days_to_expiry
...
def _auto_enter_position(trade: dict, trade_mode: str = "test"):
    ...
    position = {
        ...
        "mode": trade_mode,
        "auto_entered": True,
        "journey": [],   # [{ts, option_price, underlying_price}] — populated by tick monitor
    }
...
# CRUDEOIL: 100 barrels/lot per public MCX docs at build time — MCX revises
# lot sizes periodically, so this is only the fallback used before
# symbol_master is populated. Live trades should always resolve lot size
# from data.angel_master_contract.get_lot_size(PRIMARY_UNDERLYING).
_LOT_SIZE_FALLBACK = 100


def _resolve_lot_size(underlying: str) -> int:
    try:
        from data.angel_master_contract import get_lot_size
        return get_lot_size(underlying)
    except Exception as e:
        logger.warning(
            f"Could not resolve live lot size for {underlying} ({e}); using fallback "
            f"{_LOT_SIZE_FALLBACK}. Run: python -m data.angel_master_contract"
        )
        return _LOT_SIZE_FALLBACK


LOT_SIZE = _resolve_lot_size(PRIMARY_UNDERLYING)
    ...
    def _startup_backfill():
        try:
            if not _is_market_hours():
                return
            today = date.today()
            day_start = datetime(today.year, today.month, today.day, 9, 0, 0)
            end_dt = datetime.now()
            ...
            first = read_sql(
                ...
                "WHERE symbol = 'CRUDEOIL-I' AND timestamp::date = :d",
                {"d": today.isoformat()}
            )
            first_bar = None
            ...
            from backtest.mcx_option_resolver import get_nearest_expiry
            ...
            logger.info(f"Startup backfill: fetching CRUDEOIL-I candles from 09:15 to {end_dt.strftime('%H:%M')}...")
            bars = td_bf.fetch_historical_bars("CRUDEOIL-I", day_start, end_dt, interval="1min")
            if not bars.empty:
                for col in ["vwap", "oi"]:
                ...
                _upsert(bars)
                logger.info(f"Startup backfill: upserted {len(bars)} CRUDEOIL-I candles.")
            ...
            if nifty_close > 0:
                ...
                expiry = get_nearest_expiry(PRIMARY_UNDERLYING, today)
                if expiry:
            ...
            # ── Tick backfill (CRUDEOIL-I only — 5-day REST window) ──────────────
...
def _is_market_hours() -> bool:
    """Return True if current IST time is within MCX market hours (9:00-23:30)."""
...
def _detect_tick_gaps_today(min_gap_secs: int = TICK_STALENESS_THRESHOLD_SECS) -> list:
    """
    Find time windows in today's CRUDEOIL-I tick stream where no ticks exist
    ...
    """
    today = now_ist().date()
    session_start = datetime(today.year, today.month, today.day, 9, 0, 0)
    if now_ist() < session_start:
        return []

    df = read_sql(
        "SELECT timestamp FROM tick_data WHERE symbol = 'CRUDEOIL-I' "
...
def _backfill_ticks_if_stale():
    """
    During market hours, scan today's CRUDEOIL-I tick coverage for gaps and
    ...
    try:
        ...
        for from_dt, to_dt in gaps:
            ...
            try:
                ticks = td_bf.fetch_historical_ticks("CRUDEOIL-I", start=from_dt_pad, end=to_dt_pad)
            except Exception as e:
        ...
        _last_tick_backfill_run = now
        if total_inserted:
            logger.info(f"Tick backfill: inserted {total_inserted} CRUDEOIL-I ticks across {len(gaps)} gap(s).")
    except Exception as e:
...
def _backfill_candles_if_stale():
    """
    During market hours:
    1. If CRUDEOIL-I candles are >2 min old, fetch fresh bars from REST.
    ...
    try:
        stale = read_sql(
            "SELECT MAX(timestamp) as latest FROM minute_candles WHERE symbol = 'CRUDEOIL-I'"
        )
        ...
        # 1. Top-up CRUDEOIL-I if stale
        if need_td:
            start_dt = latest_ts.tz_convert("Asia/Kolkata").replace(tzinfo=None)
            bars = td.fetch_historical_bars("CRUDEOIL-I", start_dt, end_dt, interval="1min")
            if not bars.empty:
                for col in ["vwap", "oi"]:
                    if col not in bars.columns:
                        bars[col] = 0
                ...
                if inserted:
                    logger.info(f"Auto-backfill CRUDEOIL-I: {inserted} new candles (was {age_seconds:.0f}s stale)")
...
def scan_market():
    ...
    try:
        ...
        df = read_sql(
            ...
            "FROM minute_candles WHERE symbol = 'CRUDEOIL-I' "
        ...
        # ── Compute live micro features from last ~120s of CRUDEOIL-I ticks ──────
        ...
        try:
            tick_df = read_sql(
                ...
                "bid_qty, ask_qty FROM tick_data WHERE symbol = 'CRUDEOIL-I' "
        ...
        # Signals
        signals = generate_signals(latest, "CRUDEOIL-I")
...
def _run_end_of_day():
    ...
    1. Backfill any missing CRUDEOIL-I candles for today
    ...
    try:
        ...
        if td_eod.authenticate():
            start_dt = datetime(date.today().year, date.today().month, date.today().day, 9, 0, 0)
            end_dt = datetime(date.today().year, date.today().month, date.today().day, 15, 31, 0)
            bars = td_eod.fetch_historical_bars("CRUDEOIL-I", start_dt, end_dt, interval="1min")
            if not bars.empty:
                for col in ["vwap", "oi"]:
                    if col not in bars.columns:
                        bars[col] = 0
...
def background_scanner():
    ...
    while True:
        ...
        # End-of-day tasks: run once after 23:35 IST on weekdays (MCX closes 23:30)
        if now.weekday() < 5 and now.hour == 23 and now.minute >= 35:
            try:
                _run_end_of_day()
            except Exception as e:
                logger.error(f"EOD error: {e}")

        # Auto-start collector during market hours (9:00-23:59 IST, MCX closes 23:30)
...
def run_replay(replay_date: str):
    ...
    global replay_state
    from backtest.mcx_option_resolver import get_nearest_expiry, get_days_to_expiry, clear_cache
    ...
    try:
        ...
        ticks = read_sql(
            ...
            "FROM tick_data WHERE symbol = 'CRUDEOIL-I' AND timestamp::date = :dt "
            "ORDER BY timestamp",
            {"dt": replay_date},
        )
        if ticks.empty:
        ...
        for idx, minute_ts in enumerate(minutes):
            ...
            if not in_trade and len(candle_buffer) >= 250:  # daily_trades < 5 TEMP: disabled to collect more training data
                ...
                # Resolve option
                from backtest.mcx_option_resolver import resolve_option_at_entry
                opt = resolve_option_at_entry(
                    underlying=PRIMARY_UNDERLYING, price=latest["close"], timestamp=minute_ts,
                    direction=sig.direction,
                )
                if opt is None:
                    continue
...
def replay_page():
    ...
    days = read_sql("""
        ...
        FROM tick_data WHERE symbol = 'CRUDEOIL-I'
...
def _tick_monitor_loop():
    ...
    while True:
        try:
            # Only run during MCX hours ~9:00-23:35 IST
            now = now_ist()
                ...
                if live_price is not None and pos["status"] == "OPEN":
                    try:
                        ...
                        if (datetime.now() - last_check).total_seconds() >= REGIME_CHECK_INTERVAL:
                            ...
                            nifty_bid = float(cache.get("CRUDEOIL-I", {}).get("bid", 0) or
                                              cache.get("CRUDEOIL-I", {}).get("price", 0) or 0)
...
def _ensure_collector():
    ...
    now = now_ist()
    # Only auto-start during market hours (9:00 - 23:35 IST, weekdays; MCX closes 23:30)
    if now.weekday() >= 5 or not (9 <= now.hour < 24):
        return
        ...
        try:
            while True:
                try:
                    ...
                    # Use tick cache CRUDEOIL-I price for real-time last_price (falls back to scanner value)
                    last_price = state.get("last_price", 0)
                    nifty_tick = cache.get("CRUDEOIL-I", {})
