...
- **`broker/qty_freeze.py`** — MCX freeze-quantity order splitting, wired into `AngelOneAdapter.place_order()` (splits into multiple exchange orders when size exceeds the cap) and `PaperAdapter.place_order()` (warns, doesn't split — nothing to reject in paper mode). Pattern verified against OpenAlgo's `qty_freeze_db.py`; **`FREEZE_QTY` itself is seeded empty** — populate it from MCX's published freeze-quantity circular before trading real size, same as lot sizes.
- **Two new commodity-specific strategies** in `strategy/signal_generator.py`: `crude_inventory_volatility_breakout` (CRUDEOIL-only, trades the EIA report volatility window) and `precious_metals_trend_momentum` (GOLD/SILVER-only, multi-bar trend continuation). Both are research-grounded (see git log for sources) but **UNBACKTESTED** — no real MCX data exists yet to validate them against. Treat as hypotheses, not proven strategies.
...
- **No MCX holiday calendar.** Market-hours checks only verify weekday, not actual MCX-specific exchange holidays (which differ from NSE's). OpenAlgo tracks this in `database/market_calendar_db.py` — not yet ported here.
