...
function UnderlyingCandleChart({ candles }: { candles: Candle[] }) {
  if (!candles.length)
...
export default function ChartsPage() {
  ...
  const parseSymbol = (sym: string) => {
    // MCX option symbol: {NAME}{DDMMMYY}{STRIKE}{CE|PE}, e.g. CRUDEOIL19SEP265800CE
        ...
        <div className="t-panel p-4 mb-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: '#5a6270' }}>
              {chartView === "candles" ? "1-MIN" : "TICKS"} — {selectedDate || "..."}
          ...
          {loadingCandles || loadingTicks2 ? (
            ...
            <UnderlyingCandleChart candles={candles} />
          ) : ticks.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
